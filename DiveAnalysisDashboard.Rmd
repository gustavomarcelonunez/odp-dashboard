...
renderPlotly({
  ...
  fields$bottom_temperature_spotter <- na.approx(fields$bottom_temperature_spotter)
  fields$top_temperature_spotter <- na.approx(fields$top_temperature_spotter)

  mean_temp <- (fields$bottom_temperature_spotter + fields$top_temperature_spotter) / 2

  plot <- plot_ly(fields, x = ~timestamp) %>%
  ...
  plot_temperatures <- plot_ly(fields, x = ~timestamp) %>%
    add_trace(y = ~bottom_temperature_spotter, type = "scatter", mode = "lines", name = "Bottom Temp") %>%
    add_trace(y = ~top_temperature_spotter, type = "scatter", mode = "lines", name = "Top Temp") %>%
    add_trace(y = mean_temp, type = "scatter", mode = "lines", name = "Mean Temp", line = list(dash = 'solid'))
  
  plot_temperatures<- plot_temperatures %>% layout(
    yaxis = list(title = "Temperatures (°C)", range = c(0, max(fields$significant_wave_height_spotter))))

  plot_wave_height <- plot_ly(fields, x = ~timestamp) %>%
    add_trace(y = ~significant_wave_height_spotter, type = "scatter", mode = "markers", name = "Wave Height", marker=list(size = 4))
  
  plot_wave_height<- plot_wave_height %>% layout(
    yaxis = list(title = "Wave height (mts)", range = c(0, max(fields$significant_wave_height_spotter))))

  plot_wind_speed <- plot_ly(fields, x = ~timestamp) %>%
    add_trace(y = ~wind_speed_spotter, type = "scatter", mode = "markers", name = "Wind Speed", marker=list(size = 4))
  
  plot_wind_speed<- plot_wind_speed %>% layout(
    yaxis = list(title = "Wind speed (km/h)", range = c(0, max(fields$significant_wave_height_spotter))))
