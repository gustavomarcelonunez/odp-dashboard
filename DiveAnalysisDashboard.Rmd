...
renderPlotly({
    fields <- dataBuoy %>%
      select(timestamp, bottom_temperature_spotter, top_temperature_spotter)
      # Convierte el formato de timestamp a un objeto de fecha y hora
  ...
  fields$formatted_date <- format(fields$timestamp, format = "%d/%m/%Y")
    ...
    plots <- lapply(c("bottom_temperature_spotter", "top_temperature_spotter"), function(var) {
      plot_ly(fields, x = ~timestamp, y = ~get(var), type = "scatter", mode = "lines", name = var) 
    })
    
      plots[[1]] <- plots[[1]] %>% layout(yaxis = list(range = c(0, max(fields$significant_wave_height_spotter))))
